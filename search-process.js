...
function search() {
    var a = document.getElementById("search_input"); if ("" != a.value) {
        try {
            window.via.searchText(a.value)}catch(b) {
            alert(b)}a.value = ""; document.getElementById("search_submit").style.display = "none"; document.activeElement.blur()}return!1
}
function fadeIn(a) {
...
function initGesture(d) {
    ...
    a = 0; d.addEventListener("touchstart", function(c) {
        ...
        g || (0 < a/*&&(document.body.style.position="fixed")*/, a = c.touches[0].pageY-e, 0 < a && 98 > a && (f = Math.max(.1, 1-Math.min(1, a/98)), b.style.opacity = f, b.style.transform = "translateY("+a+"px)"))}, !1); d.addEventListener("touchend", function(c) {
        /*document.body.style.position="static";*/if (0 != a) {
            if (88.2 < a)try {
                window.via.cmd(514)}catch(h) {}b.style.transform = "translateY(0px)"; fadeIn(b)}},
        !1)};
